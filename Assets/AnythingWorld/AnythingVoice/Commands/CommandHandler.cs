    ...
    public class CommandHandler
    {
        ...
        private static void RemoveModel(params Model[] modelList)
        {
            foreach (var model in modelList)
            {
                ...
                {
                    if (TryFindAWObjectInScene(model.name, out var foundObject))
                    {
                        Utilities.Destroy.GameObject(foundObject);
                    }
                    else
                    {
                        break;
        ...
        private static void MoveModel(params Model[] modelList)
        {
            ...
            foreach (var model in modelList)
            {
                ...
                {
                    if (TryFindAWObjectInScene(model.name, out var gameObject))
                    {
                    ...
                    {
                        Debug.Log($"Could not find {model.name} in scene.");
                    }
        ...
        private static Vector3 GetNewRelativePosition(Model model)
        {
            ...
            else if (TryFindAWObjectInScene(model.location.model_name, out GameObject relativeToGameObject))
            {
        ...
        /// <returns></returns>
        private static bool TryFindAWObjectInScene(string _modelName, out GameObject gameObject)
        {
            ...
            foreach (var model in activeModels)
            {
                if (model?.name!= null)
                {
                    if (model.name.Contains(_modelName))
                    {
                        gameObject = model.gameObject;
                        return true;
                    }
